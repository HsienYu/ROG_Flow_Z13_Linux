...
# Installation state tracking (from original.sh)
...
install_base_system() {
    ...
    local base_packages="base linux-zen linux-firmware base-devel vim nano networkmanager git wget curl intel-ucode amd-ucode zsh grub efibootmgr mesa xf86-video-amdgpu linux-zen-headers"
...
if [[ "$INSTALL_POWER_MGMT" == "y" ]]; then
    ...
    systemctl enable power-profiles-daemon; systemctl enable tlp; systemctl enable asusd.service
